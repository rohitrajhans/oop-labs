...
class Customer {
	...
	JScrollPane sp1;
	JButton savebtn, resetbtn, editbtn1, editbtn2, deletebtn, showbtn;
	JComboBox<String> tf3, tf9;
	JScrollPane jsp;
	JTable table;
...
//		p4 = new JPanel(new FlowLayout(FlowLayout.LEFT));
		...
		showbtn = new JButton("Show entries");
...
//				tf3.setSelectedItem("INDIA"); // Variant
				tf4.setText("");
			}
		});
...
//		}
		...
		deletebtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				if( x==0 ) {
					try {
						int value = Integer.parseInt(tf5.getText());
						Connection con = getConnection();
						PreparedStatement st = con.prepareStatement("delete from CUSTOMER_INFO where CUSTOMER_ID=?");
						st.setInt(1, value);
						st.executeUpdate();
						JOptionPane.showMessageDialog(p3, "Successfully deleted record with id = " + value);
						con.close();
					} catch(ClassNotFoundException E) {
						JOptionPane.showMessageDialog(p3, "Unable to delete record");
		...
		showbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				try {
					...
					JOptionPane.showMessageDialog(f, "Could not display entries");
				}
			}
		});
	}
